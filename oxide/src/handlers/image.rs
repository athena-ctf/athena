...
/// List images
pub async fn list() -> ApiResult<Json<Vec<ContainerImage>>> {
...
pub async fn retrieve(Path(name): Path<String>) -> ApiResult<Json<ContainerImage>> {
    let images = IMAGE_PUSH_STATE.read().await;

    images.get(&name).map_or_else(
        || Err(ApiError::NotFound("Image not found".to_owned())),
