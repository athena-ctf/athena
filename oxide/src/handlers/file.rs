...
single_relation!(File, Challenge);
...
pub async fn upload(
    ...
    let (sha1_hash, backend) = if let Some(aws_s3) = &settings.file_storage.aws_s3 {
        let resp = state
            ...
            .send()
            .await
            .map_err(|err| AthenaError::Generic(err.to_string()))?;
        ...
        return Err(ApiError::BadRequest(
            AthenaError::InvalidConfig("file storage".to_owned()).to_string(),
        ));
    };
...
pub async fn download_by_id(
    ...
    .await?
        ...
        return Err(ApiError::NotFound("File does not exist".to_owned()));
    };
