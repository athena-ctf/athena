...
export function DeploymentTable() {
  ...
  const onFileUpload = async (file: File) => {
    apiClient.POST("/admin/deployment/import", {
      body: {
        file,
      },
      bodySerializer: (body) => {
  ...
  useEffect(() => {
    apiClient.GET("/admin/deployment").then((res) => {
      if (res.error) {
        ...
        apiClient
          ...
          .then((res) => {
            if (res.data) {
              ...
              a.href = url;
              a.download = `deployment.${selected}`;
    ...
    <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="overflow-y-auto max-h-screen">
          ...
          <DeploymentForm kind="create" onSuccess={(model) => setData([...data, model])} />
        </DialogContent>
      </Dialog>
