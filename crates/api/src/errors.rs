...
pub enum Error {
    ...
    Fs { source: std::io::Error, path: String },
...
impl IntoResponse for Error {
    fn into_response(self) -> axum::response::Response {
        ...
        match self {
            Self::BadRequest(message) => (StatusCode::BAD_REQUEST, Json(JsonResponse { message })).into_response(),

            Self::NotFound(message) => (StatusCode::NOT_FOUND, Json(JsonResponse { message })).into_response(),

            Self::Unauthorized(jar, message) => {
                if let Some(jar) = jar {
                    (StatusCode::UNAUTHORIZED, jar, Json(JsonResponse { message })).into_response()
                } else {
