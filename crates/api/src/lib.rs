...
pub struct AppState {
    ...
    pub rbac_manager: Arc<rbac::Manager>,
    pub fileserver_manager: Arc<fileserver::Manager>,
...
pub async fn start(settings: Settings) -> Result<()> {
    ...
    scheduler
        .add(Job::new_repeated_async(Duration::from_secs(300), move |_, _| {
    ...
    let rbac_manager =
        Arc::new(rbac::Manager::new(redis_client.clone(), rbac::get_defaults(), routes::get_url_mappings()).await?);

    let fileserver_manager = Arc::new(fileserver::Manager::new(
        LocalFileSystem::new_with_prefix(&settings.file_storage.local.path)?.with_automatic_cleanup(true),
        AmazonS3Builder::new()
            .with_access_key_id(&settings.file_storage.backup.access_key_id)
            .with_secret_access_key(&settings.file_storage.backup.secret_access_key)
            .with_region(&settings.file_storage.backup.region)
            .with_bucket_name(&settings.file_storage.backup.bucket_name)
            .build()?,
    ));
    ...
    std::fs::write(&location, serde_json::to_vec(&*settings.read().await)?).map_err(|err| Error::Fs {
        source: err,
        path: location,
    })?;
