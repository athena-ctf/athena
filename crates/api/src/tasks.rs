...
use crate::schemas::{Award, Challenge, ChallengeKindEnum, Deployment, Instance, Player, Submission};
use crate::{docker, leaderboard};

pub async fn load_leaderboard(db: &DbConn, leaderboard_manager: &leaderboard::Manager) -> Result<()> {
...
pub async fn load_player_updates(db: &DbConn, pool: &Pool) -> Result<()> {
    for (player_id, updated_at) in Player::find()
        .select_only()
        .columns([entity::player::Column::Id, entity::player::Column::UpdatedAt])
        .into_tuple::<(Uuid, DateTimeWithTimeZone)>()
        .all(db)
        .await?
    {
        pool.hset::<(), _, _>(PLAYER_LAST_UPDATED, (player_id.to_string(), updated_at.timestamp()))
            .await?;
    }
        ...
        .await?
    {
        if let Some(deployment_model) = deployment_model {
            for instance_model in deployment_model.find_related(Instance).all(db).await? {
                if !matches!(
                    manager.get_container_status(&instance_model.container_id).await?,
                    ContainerStateStatusEnum::RUNNING | ContainerStateStatusEnum::RESTARTING
                ) {
                    manager.restart_container(&instance_model.container_id).await?;
                }
            }
        } else {
            manager.deploy_challenge(&challenge_model, None).await?;
