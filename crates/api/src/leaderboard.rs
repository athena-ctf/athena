...
impl Manager {
    ...
    fn get_score(&self, points: i32) -> f64 {
        f64::from(points * 10i32.pow(self.num_digits)) + (self.end_timestamp - Utc::now().timestamp()) as f64
    }
    ...
    pub async fn load_players(&self, db: &DbConn) -> Result<()> {
        ...
        for player in players {
            let challenge_points = player
                ...
                .select_only()
                .column_as(Expr::col(entity::challenge::Column::Points).sum(), "challenge_points")
            ...
            let award_prizes = player
                .find_related(Award)
                .select_only()
                .column_as(Expr::col(entity::award::Column::Prize).sum(), "award_prizes")
                .into_tuple::<(i32,)>()
                .one(db)
                .await?
                .unwrap()
        ...
        Ok(self
            .redis_pool
            .zadd(PLAYER_LEADERBOARD, None, None, false, false, leaderboard_player)
            .await?)
    }
    ...
    pub async fn top_10_players(&self) -> Result<Vec<Ranking>> {
        Ok(self
            ...
            .await?
            .into_iter()
            .map(|(k, v)| Ranking { member: k, score: v })
            .collect())
    }
    ...
    pub async fn init_player(&self, id: Uuid) -> Result<()> {
        Ok(self
            .redis_pool
            .zadd(PLAYER_LEADERBOARD, None, None, false, false, (0.0, id.to_string()))
            .await?)
    }
    ...
    pub async fn init_team(&self, id: Uuid) -> Result<()> {
        Ok(self
            .redis_pool
            .zadd(TEAM_LEADERBOARD, None, None, false, false, (0.0, id.to_string()))
            .await?)
    }
    ...
    pub async fn team_rank(&self, id: Uuid) -> Result<(u64, i32)> {
        let (rank, score) = self.redis_pool.zrevrank(TEAM_LEADERBOARD, id.to_string(), true).await?;
    ...
    pub async fn player_history(&self, id: Uuid) -> Result<Vec<PointsHistory>> {
        Ok(self
            .redis_pool
            .zrange::<Vec<(i64, f64)>, _, _, _>(Self::player_history_key(id), 0, -1, None, false, None, true)
            .await?
            .into_iter()
    ...
    pub async fn team_history(&self, id: Uuid) -> Result<Vec<PointsHistory>> {
        Ok(self
            .redis_pool
            .zrange::<Vec<(i64, f64)>, _, _, _>(Self::team_history_key(id), 0, -1, None, false, None, true)
            .await?
            .into_iter()
            .map(|(points, timestamp)| PointsHistory {
                timestamp: timestamp as i64,
