...
impl Manager {
    pub async fn new(
        ...
        for (role, permissions) in roles {
            redis_pool
                .sadd::<(), _, _>(role_key(role), permissions)
                .await?;
        }
    ...
    pub async fn can_do_action(
        ...
        Ok(self
            ...
            || self
                .redis_pool
                .sismember(override_key(id), permission)
                .await?)
    }

    pub async fn add_role(&mut self, role: String, permissions: Vec<String>) -> Result<()> {
        Ok(self.redis_pool.sadd(role_key(role), permissions).await?)
    }

    pub async fn add_override(&mut self, id: Uuid, permission: String) -> Result<()> {
